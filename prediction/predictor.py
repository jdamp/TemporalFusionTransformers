...
class Predictor:
    ...
    def build_n_months_ahead_predictions(
        ...
        if add_suffix:
            df_result.columns = [f"{col}_{country}_{n_months_ahead}" for col in df_result.columns]
        return df_result
...
def build_model_benchmark_df(
    ...
    for country in countries:
        ...
        for n_months_ahead in months_ahead_vals:
            dfs.append(
                plot.generate_ar_forecast(ar_model, country, start_date, end_date, n_months_ahead)
            )

            dfs.append(
                predictor.build_n_months_ahead_predictions(n_months_ahead, country, add_suffix=True)
            )
        truths = tft.df_target_12m_pct.loc[start_date:end_date, country].rename(f"truth_{country}")
        dfs.append(truths)
